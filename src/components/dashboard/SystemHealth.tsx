...
import { Server, Wifi, Database, Shield, Activity } from "lucide-react";
          ...
          return (
            <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-vytreon-dark/50">
              <div className="flex items-center space-x-3">
                <Icon className="text-vytreon-cyan" size={16} />
                <div>
                  <p className="text-sm font-medium text-white">{service.name}</p>
                  <p className="text-xs text-gray-400">{service.uptime} • {service.latency}</p>
                </div>
              </div>
